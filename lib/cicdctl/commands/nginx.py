...
from ..utils.nginx.driver import NGinxDriver

"""cicdctl nginx <command> <cluster-name:workspace> [tf-options|tf-vars]"""
...
for command in [command for command in commands(__file__) if command in [
    ...
    def bind_command(command):
        ...
        def command_func(settings, instance, flags):
            ...
            driver_method = getattr(NGinxDriver(settings, instance, flags=flags), command)
            driver_method()
        command_func.__name__ = command
    
        nginx.command(name=command, context_settings=PASS_THRU_FLAGS)(command_func)
